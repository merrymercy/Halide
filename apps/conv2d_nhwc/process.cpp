...
#include "conv2d_nhwc.h"
#include "conv2d_nhwc_classic_auto_schedule.h"
#include "conv2d_nhwc_auto_schedule.h"
...
int main(int argc, char **argv) {
    ...
    Buffer<float> filter(CO, CI / groups, KW, KH);
    ...
    three_way_bench(
        [&]() { conv2d_nhwc(input, filter, output); },
        [&]() { conv2d_nhwc_classic_auto_schedule(input, filter, output); },
        [&]() { conv2d_nhwc_auto_schedule(input, filter, output); }
    );

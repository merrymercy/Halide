...
public:
    ...
    void generate() {
        ...
        RDom r(0, KW, 0, KH, 0, CI / groups);

        f_conv(c, w, h, n) = 0.0f;
        f_conv(c, w, h, n) += filter(c, r.z, r.x, r.y)
            * padded(c / (CO / groups) * (CI / groups) + r.z, w * SW + r.x * DW - PW, h * SH + r.y * DH - PH, n);
        ...
        filter.dim(0).set_bounds(0, CO).set_stride(1)
              ...
              .dim(2).set_bounds(0, KW).set_stride(CO * (CI / groups))
              .dim(3).set_bounds(0, KH).set_stride(CO * (CI / groups) * KW);
...
HALIDE_REGISTER_GENERATOR(Conv2d, conv2d_nhwc)
